...
YourKit是一款业内领先的性能分析工具，目前支持Java和.NET两个平台。该工具功能全面强悍，能通过本地连接或者远程连接的方式，对各种服务器，框架，平台的业务进行分析，并提供了多种由浅入深，适合开发环境或者生产环境的分析模式。该工具提供了高效的图形化显示方式，动动鼠标就可以对系统进行显微镜式的观察分析。
...
YourKit提供了两种方式连接到Java进程，一种是attach方式，另一种是integrate方式。



## 通过attach方式连接到远程服务器

在控制台，通过attach方法可以通过进程号，直接连接到运行中的任何Java进程中，这种方式并不保证总能连接成功，而且在会禁用某些分析功能。
